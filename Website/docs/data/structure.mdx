---
id: Data structure
description: How to structure data?
slug: /data-structure
title: 🏗️ Structure
---

## File based structure
Data grouped based on a type of file. One file is storing the data in a logical organised format. <br />

:::tip example
`-` Spreadsheets <br />
AKA - Flat file
:::

## Directory based structure
Data in files within seperate organised folders. Folders tend to be hierachical. 

:::tip example
Like a tree
:::

### Relational database model
Organises data into table, which can be linked based on data common to each. It allows you to retrieve an entirely new table from one or more tables with a single query. 

### Advantages
`-` Flexibility <br />
ㅤ`-` Allows you to make changes while it's running <br />
`-`  Ease of backup and disaster recovery  <br />
`-` Speed  <br />
`-` Security <br />
`-` Multi-User  <br />
`-` Organised data in seperate tables  <br />

### Disadvantages
`-` Cost  <br />
ㅤ`-` The cost is expensive. There needs to be seperate software <br />
`-` Can be slow <br />
ㅤ`-` Based on storage <br />
`-` Can be complex <br />
ㅤ`-` Vast amounts of data, lots of information to be stored. <br />
ㅤㅤ`-` Many tables