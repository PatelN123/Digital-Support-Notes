...
function transform(variants) {
  return (dir, node, context) => {
    ...
    const { exp } = dir;
    if (exp) {
      console.log(compilerCore.createObjectProperty("style", exp));
    }
    const prop = compilerCore.findProp(node, "initial", true, false);
    if (!isBindDirective(prop) || prop.exp == null || prop.arg == null) {
      return result;
    }
    result.props = [
      compilerCore.createObjectProperty(`style`, createStyleObjectExpressionFromDirectiveNode(prop, node, context))
    ];
    return result;
  };
}
...
function mapNodeContentHanlder(value) {
  ...
  } else if (compilerCore.isText(value)) {
    if (core.isString(value.content)) {
      return value.content;
...
function createStyleObjectExpressionFromDirectiveNode(prop, node, context) {
  ...
  if (isSimpleExpressionNode(prop.exp)) {
    if (isConstant(prop.exp)) {
      ...
      if (status === "ok") {
        for (const [key, val] of Object.entries(value)) {
          properties.push(compilerCore.createObjectProperty(key, compilerCore.createSimpleExpression(String(val), true, prop.loc, 3)));
        }
      }
      return compilerCore.createObjectExpression(properties, node.loc);
    } else {
      if (isSimpleExpressionNode(prop.arg) && isConstant(prop.arg)) {
        const source = compilerCore.createSimpleExpression(prop.exp.content, false, prop.loc, 0);
        const from = compilerCore.createObjectExpression([], prop.loc);
        return compilerCore.createCallExpression(context.helper(compilerCore.MERGE_PROPS), [source, from], node.loc);
      } else {
        return compilerCore.createObjectExpression(properties, node.loc);
      }
    }
    ...
    const source = compilerCore.createSimpleExpression(expression, false, prop.loc, 0);
    const from = compilerCore.createObjectExpression([], prop.loc);
    return compilerCore.createCallExpression(context.helper(compilerCore.MERGE_PROPS), [source, from], node.loc);
  } else {
    return compilerCore.createObjectExpression(properties, node.loc);
